...
<div class="row align-items-center py-4">
  ...
  <div class="col-lg-6 col-5 text-right">
    ...
    <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#subkriteriaAnggota">
      <span class="btn-inner--icon"><i class="fas fa-plus"></i></span>
      <span class="btn-inner--text">Tambah</span>
    </button>
    @endif
    <!-- Modal Tambah-->
    <div class="modal fade" id="subkriteriaAnggota" tabindex="-1" role="dialog" aria-labelledby="subkriteriaKetuaLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="subkriteriaKetuaLabel">Data Sub Kriteria Anggota</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form id="subkriteriaAnggotaForm" enctype="multipart/form-data">
              @csrf
...
<div class="card-header">
  <h3 class="mb-0">Data Tabel Sub Kriteria Anggota</h3>
...
$('#subkriteriaAnggotaForm').submit(function(e) {
  e.preventDefault();
...
$('#subkriteriaAnggotaForm').validate({
  rules: {
    kriteriaketua_id: {
      required: true
    },
...
function editsubKriteria(id) {
  var url = '{{route("SubAnggota.getbyid",":id")}}'
  url = url.replace(':id', id)
  $.get(url, function(subkriteriaAnggota) {
    $("#id").val(subkriteriaAnggota.id);
    $("#sub_update").val(subkriteriaAnggota.sub_kriteria);
    $("#nilai_update").val(subkriteriaAnggota.nilai);
    $("#keterangan_update").val(subkriteriaAnggota.keterangan);
...
function deletesubKriteria(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = '{{route("SubAnggota.delete",":id")}}'
      url = url.replace(':id', id)
      $.ajax({
        url: url,
        type: 'DELETE',
